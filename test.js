...
;(async () => {
  ...
  test('Pi test', async (_) => {
    ...
    await page.waitForSelector('#block-id-0') // Wait for blocks to load
    await page.click('.run-button')
    await page.waitForSelector('.chart') // Wait for resulting charts to load
    const value = await page.$eval('h1', node => node.innerText)
